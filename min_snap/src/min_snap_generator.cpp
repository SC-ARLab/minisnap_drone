...
void eight_goal_cb(const geometry_msgs::PoseStamped target_info)
{
    goal_pt.position = odom.pose.pose.position;
    goal_pt.position.z = GOAL_HEIGHT;
    std::vector<Eigen::Vector2d> list{{0.0 , 0.0 },\
                                      {2.0 , -1.0},\
                                      {2.0 , 1.0 },\
                                      {0.0 , 0.0 },\
                                      {-2.0, -1.0},\
    ...
    for (size_t i=0; i<list.size(); i++)
    {
        ...
        tmp_pt.position.x += list[i][0]*scale; 
        tmp_pt.position.y += list[i][1]*scale;
...
int main(int argc, char **argv)
{
    ...
    eight_goal_sub = nh.subscribe("/rviz_goal", 10, eight_goal_cb);
